...
/// 标记已读后 syncSessionSummariesFromDevices 数量没变 —— 回归测试
///
/// 问题场景：
/// 1. Device A 有 3 条未读消息
/// 2. Device A 标记已读 → unread_count = 0
/// 3. Device B 还未同步到已读状态，其摘要中 unread_count = 3
/// 4. Device A 调用 syncSessionSummariesFromDevices（从 Device B 拉取摘要）
/// 5. upsertFromRemote 使用 MAX(local, remote) 策略 → MAX(0, 3) = 3
/// 6. Device A 的 unread_count 从 0 变回 3 ❌
///
/// 根本原因：
...
void main() {
  ...
  setUp(() async {
    _testCounter++;
    final base =
        '${Directory.systemTemp.path}/wenzagent_mark_read_sync_test_$_testCounter';
  ...
  group('标记已读后同步数量不变（回归测试）', () {
    test('场景1：本地标记已读后，从远程同步不应恢复未读数', () {
      ...
      // （Device B 的摘要是在步骤2同步过来的，此时还是3）
      expect(storeB.getUnreadCount('emp-1', deviceId: deviceA), equals(3));

      // ---- 步骤5: Device A 调用 syncSessionSummariesFromDevices ----
      // 从 Device B 拉取摘要 → upsertFromRemote → MAX(0, 3) = 3 ❌
      syncBToA('emp-1');
    ...
    test('场景3：双向同步后标记已读，再次同步不应恢复', () {
      ...
      syncAToB('emp-1'); // A(0) → B: MAX(5, 0) = 5, B不变
      syncBToA('emp-1'); // B(5) → A: MAX(0, 5) = 5 ❌
  ...
  group('正常同步流程（不受影响）', () {
    ...
    test('两端都有未读时，同步取 MAX 是正确的', () {
      ...
      // 注意：必须用不同的 deviceId，否则同 employeeId+deviceId 的 UPSERT 会冲突
      for (int i = 1; i <= 5; i++) {
      ...
      // 双向同步：各自保留原始 deviceId
      syncAToB('emp-1'); // A(deviceA) → B: B 新增一条 (emp-1, deviceA) 摘要
      syncBToA('emp-1'); // B(deviceB) → A: A 新增一条 (emp-1, deviceB) 摘要
